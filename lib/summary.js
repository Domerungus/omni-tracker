...
function printBuildSummary(sessionResults) {
  ...
  for (const entry of sessionResults) {
    if (entry.url) {
      tgText += `• ${entry.name}: <a href="${entry.url}">${entry.current.toFixed(2)} € @ ${entry.shop_name}</a> (Цель: ${entry.target} €)\n`;
    } else {
      tgText += `• ${entry.name}: <b>${entry.current.toFixed(2)} €</b> (Цель: ${entry.target} €)\n`;
    }
  }
  tgText += `\n`;

  // Находим самую выгодную сделку (наибольшую разницу или скидку)
  let bestDeal = null;
  ...
  if (bestDeal) {
    const savingsText = maxSavings > 0 ? `экономия: ${maxSavings.toFixed(2)} €! 🔥` : `разница: ${maxSavings.toFixed(2)} €`;
    tgText += `🔥 <b>САМАЯ ВАЖНАЯ ССЫЛКА (Лучшая скидка):</b>\n`;
    tgText += `👉 <a href="${bestDeal.url}">${bestDeal.name} за ${bestDeal.current.toFixed(2)} € @ ${bestDeal.shop_name}</a> (${savingsText})\n\n`;
  }
